...
# Paths relative to project root -- run scripts with setwd() at project root
data_path <- "data"
...
# Treat 1:
# Loading data file
raw_data <- read.csv(paste0(data_path, "/raw_data.csv"))
df_wide <- raw_data

# Loading data file with times per page
raw_times <- read.csv(paste0(data_path, "/raw_times.csv"))

# Treat 2:
# Loading data file additional treatment
raw_data_extra <- read.csv(paste0(data_path, "/raw_data_extra.csv"))
...
# Loading data file with times per page
raw_times_extra <- read.csv(paste0(data_path, "/raw_times_extra.csv"))


# Preparation for merge
######################################################

# Participant id (handle BOM prefix present in older R versions)
names(df_wide)[names(df_wide) %in% c("ï..participant.id_in_session", "participant.id_in_session")] <- "id"
names(df_wide_extra)[names(df_wide_extra) %in% c("ï..participant.id_in_session", "participant.id_in_session")] <- "id"
...
# Group by subject to prevent lag() from crossing subject boundaries
df_long <- arrange(df_long, id, round) %>% group_by(id)
...
cat(">> Building history variables...\n")
...
cat(">> Computing posteriors & analysis variables...\n")
...
df_long <- mutate(df_long, belief_lag1 = ifelse(round > 1, lag(belief, 1), 0.5))
df_long <- mutate(df_long, belief_lag2 = ifelse(round > 2, lag(belief, 2), 0.5))
df_long <- mutate(df_long, belief_lag3 = ifelse(round > 3, lag(belief, 3), 0.5))
...
# Re-group by subject for remaining lag operations
df_long <- arrange(df_long, id, round) %>% group_by(id)

df_long <- mutate(df_long, belief_diff_abs_lag = lag(belief_diff_abs, 1))
...
# lagged variables
df_long <- mutate(df_long, post_lag1 = lag(posterior_subj, 1))
df_long <- mutate(df_long, post_lag2 = lag(posterior_subj, 2))

df_long <- mutate(df_long, belief_change_lag1 = lag(belief_change, 1))
df_long <- mutate(df_long, belief_change_adj_lag1 = lag(belief_change_adj, 1))

df_long <- mutate(df_long, obslnpost_lag1 = lag(obslnpost, 1))
df_long <- mutate(df_long, truelnpost_lag1 = lag(truelnpost, 1))

df_long <- mutate(df_long, signal_subj_lag1 = lag(signal_subj, 1))

df_long <- mutate(df_long, prior_ratio_lag = lag(prior_ratio, 1))
...
df_long <- mutate(df_long, correct_lag = lag(correct, 1))
df_long <- mutate(df_long, under_lag = lag(under, 1))
df_long <- mutate(df_long, over_lag = lag(over, 1))
df_long <- mutate(df_long, no_change_lag = lag(no_change, 1))
df_long <- mutate(df_long, wrong_lag = lag(wrong, 1))

df_long <- mutate(df_long, over_report_lag1 = lag(over_report, 1))
df_long$over_report_lag1 <- ifelse(df_long$round==1, NA, df_long$over_report_lag1)

df_long <- mutate(df_long, over_report_lag2 = lag(over_report, 2))
df_long$over_report_lag2 <- ifelse(df_long$round==1 | df_long$round==2, NA, df_long$over_report_lag2)

df_long <- mutate(df_long, belief_diff_lag1 = lag(belief_diff, 1))
df_long$belief_diff_lag1 <- ifelse(df_long$round==1, NA, df_long$belief_diff_lag1)

df_long <- mutate(df_long, belief_diff_lag2 = lag(belief_diff, 2))
...
df_long <- mutate(df_long, belief_lag6 = lag(belief, 6))
df_long <- mutate(df_long, belief_lag4 = lag(belief, 4))
...
df_long <- mutate(df_long, post_induced = ifelse(post_induced < 0, 0, post_induced))
...
df_long <- mutate(df_long, prev_ball = lag(ball_red, 1))
...
cat(">> Creating subject-level datasets...\n")
...
write.csv(df_regular, file = paste0(processed_path, "/data_regular.csv"), row.names = FALSE)

# Saving data file for info retraction
save(df_retract, file = paste0(processed_path, "/data_retract.rda"))
write.csv(df_retract, file = paste0(processed_path, "/data_retract.csv"), row.names = FALSE)

# Saving data file for info confirmation
save(df_confirm, file = paste0(processed_path, "/data_confirm.rda"))
write.csv(df_confirm, file = paste0(processed_path, "/data_confirm.csv"), row.names = FALSE)

# Saving data file for informative signals
save(df_informative, file = paste0(processed_path, "/data_informative.rda"))
write.csv(df_informative, file = paste0(processed_path, "/data_informative.csv"), row.names = FALSE)
