# Known issues — code vs. published paper

This file records the points where the replication code intentionally
differs from the numbers or figures as printed in the paper. In every case
the code is internally consistent with the paper's stated methodology
(Section 4.2: Grether regressions estimated **without an intercept**); the
printed paper has small inconsistencies introduced during revisions.

## 1. Table B1 — estimated without an intercept
...
## 2. Table B2 — H(0,0) baseline definition
...
## 3. Table 1 — Initial column standard errors
...
## 4. Theory-prediction curves are not generated
...
added by hand in the paper's LaTeX. Each generated TikZ figure ends with a
`% ADD THEORY CURVE BELOW` placeholder marking where the curve is inserted.

## 5. stargazer 5.2.3 compatibility patch

`code/utils/packages.R` patches stargazer 5.2.3 for R ≥ 4.2 (the released
stargazer fails on newer R). This is applied automatically.
